...
| 2026-08-10 | fix | **5.9.6-C1: `_apply_enforce_filter` 修复 — 增加 `customer_label_occurrences` 过滤**。原先只过滤 `aspects_json.aspects`（V3 旧路径，数组为空），导致 enforce 模式下 registry 拒绝的 label 仍出现在客户前台 TOP10 展示中。修复后同时过滤 `aspects_json.customer_label_occurrences`（V2 实际数据路径），通过 `canonical_label_key` 匹配 `rejected_keys`，设置 `display_allowed=False` + `registry_rejected=True`。`_project_customer_label_occurrence`（line 2419）已有 `display_allowed is False` 跳过逻辑。 | 生产验证进行中 |
