...
| 日期 | 问题描述 | 解决方案 |
|------|---------|---------|
| 2026-08-13 | **阶段 A 收尾（A-4 抽检结论落规则）：size 域粗细标签统一 + #389/#368 人工纠错**。Erika A-4 闸门 2 抽检判定：① `runs_too_small`/`runs_too_large` 太细（"分到底是下水库上半身小了还是鞋小了"），统一为粗标签 `size_fit_problem`，`inaccurate_size_chart` 保留独立；② #389 "Already leaking after only a few uses. Disappointed." 原标 `breaks_easily` 是错标（"leaking" 是进水语义），应为 `water_leaks_through`；③ #368 "Excellent customer service" 应输出前台标签 `customer_service_helpful`。硬约束：不做类目硬编码，全部走通用语义规则 + LLM aspect 投影。 | ① size 统一：`_issue_from_rules()`/`_ALLOWED_ASPECT_KEYS_BY_LABEL`/`_SPECIFIC_ISSUE_ZH_BY_KEY` 删除 `runs_too_small`/`runs_too_large`，统一到 `size_fit_problem`；`label_deduplication.COARSE_TO_FINE_LABELS` 清空（粗细去重关系作废，D1 语义守卫保留）。② #389 纠错：删除 3 处把 "already leaking after only a few uses" 误当 `breaks_easily` 正证据 / 当 `water_leaks_through` "durability-only" 阻断的 stale 规则（`_is_valid_breaks_easily_issue_evidence` / `_is_durability_only_leak_context` / breaks_easily content-rule），该句现落到 `water_leaks_through`。③ #368 增标签：`_highlight_from_rules()` 新增 `customer_service` aspect → `customer_service_helpful` 映射 + `L2_CATEGORY_SAFE_LABEL_KEYS["highlight"]` 加白（走 LLM 投影路径，不写内容规则，避免类目硬编码）。验收：351_400 gold focus label fp/fn 全 0；#389 → `water_leaks_through`、#368 → `customer_service_helpful`；全量 backend 测试 532 passed / 18 failed（18 条均为 overall_satisfied 口径 + review_fragment 多模块等**既有**失败，A-5 未引入新失败）。 |
| 2026-08-12 | **阶段 A Task A-3：证据感知粗细标签去重 + D1 语义守卫**。A-2 基线中 3 条极性反标含两条粗细标签并存（row-366/386 `size_fit_problem` + `runs_too_small`，同 evidence 不同 aspect_key 来自 LLM vs content-rule 独立路径），一条真误标（row-378 `not_breathable` 由全文规则 sweat 词触发，实际出汗来自厨房/烤箱环境高温而非产品透气性差）。根因：① 两条独立路径产生 `fit` 标签无跨路径去重（LLM aspect 投影路径 → `_normalize_aspect_issue()` → `_issue_from_rules()` 和全文 content-rule 路径 → `_waders_issue_rule_occurrences()`）；② `_has_frontstage_key()` 只做 exact canonical_key 去重，不处理粗细关系；③ 规则引擎无法区分出汗的环境原因 vs 产品原因。 | 新建 `backend_api/app/services/label_deduplication.py`：① 证据感知粗/细去重 —— `COARSE_TO_FINE_LABELS` 关系映射（第一版仅启用 A-2 已验证的 `size_fit_problem` → `runs_too_small`/`runs_too_large`），5 条件去重（同 label_type + 兼容 aspect_key + 证据重叠 + 细标签置信度 ≥ 粗标签 + 不同源或严格更高置信度），兼容 aspect_key 家族（size_fit/boot_fit/mobility/comfort 互通），来源权威性检查（防同源同置信度误删 gold 期望标签如 #395）；② D1 语义守卫 —— 通用条件否决因环境高温出汗误标的 `not_breathable`（出汗词 + 环境高温上下文 + 无明确产品透气性否定），不做 row ID 硬编码；③ 统一入口 `apply_label_postprocessing()`（先 guard 再 dedup）。接入点：`iter_specific_issue_occurrences()` 和 `iter_customer_highlight_occurrences()` 的 10 个 return 语句均通过 `_apply_label_postprocessing()` 统一后处理。bump `CUSTOMER_LABEL_OCCURRENCE_RULESET_VERSION` 至 `2026-08-12-a3-coarse-fine-dedup+d1-semantic-guard`。kill switch：`COARSE_FINE_DEDUP_ENABLED` / `SEMANTIC_GUARD_ENABLED`（默认 true）。CI `six-metrics-gate.yml` 新增 `label_deduplication.py` 触发路径。三场景验收对比：FP 14→13（D1 guard 否决 row-378 误标），total_system 73→71（去重消除 2 个粗标签），极性反标保持 3（row-366/386 非重叠 evidence 从不同 aspect 保留粗标签属正确行为）。新增单元测试 36 个全绿，现存回归 13 个全绿，ruff clean。 |
| 2026-08-10 | **5.9 阶段 A 验收基线第 3 次测量（模块流向正确率）**。Erika 完成 Excel 标注表 `docs/5.9-阶段A-标签+模块流向标注表.xlsx`（95 行证据片段 + 模块流向 Y/N），fixture 新增 `canonical_key` + `module_routing` + `module_routing_summary` 字段；脚本 `acceptance_six_metrics.py` 新增 `_module_flow_accuracy()` 计算模块流向正确率。 | 第 3 次基线：错标率 16.1%（FP=14/87）、漏标率 20.6%（FN=19/92）、极性反标 3、串台 0、**模块流向正确率 75.0%**（51/68 TP）。新发现：audit_filter 违规 10 条（`overall_satisfied` 简短好评如 "Work great"/"Excellent" 被前台输出但 gold 标为应过滤），consumer_profile + unmet_need 系统未产生无法测量。基线报告 `docs/5.9-阶段A-验收基线首次测量.md` 已更新。 |
| 2026-08-10 | **5.9 阶段 A 验收基线建立（第 2 次修正）**。修正内容：① `allowed_aspects` 从 `None`（回退到硬编码 ASPECT_KEYS 21 个通用 key，不含 waterproof/breathability/grip 等 waders 专属 key）改为 `[a["key"] for a in taxonomy_aspects]`（与 production workers/jobs.py:527 一致）；② 极性反标定义从一刀切（rating≤2出highlight或≥4出issue即算）改为 gold 没标+系统标了反极性才算；③ 新增 waders 21 个 key 的硬编码 fallback（CI 无 DB 时使用，与 DB taxonomy 同步）。 | 第 2 次基线：错标率 16.1%（FP=14/87）、漏标率 20.7%（FN=19/92，❌ 首次未过线）、极性反标 3（修正定义后从 11→3）、串台 0。新发现：① LLM 零输出 `waterproof` aspect key（waders 最核心维度缺失，导致漏标率偏高）；② `stability` 不在 waders taxonomy 中，2 条评论完全失败（4%）；③ 模块流向标注表 `docs/5.9-阶段A-模块流向标注表.md` 待 Erika 填写。基线报告 `docs/5.9-阶段A-验收基线首次测量.md` 已更新。未 push，等 Erika 确认。 |
|------|---------|---------|
| 2026-08-10 | **规则层 regex 极性误匹配（5.9.7-T4 Layer 0）**。content rule 的 regex 只看孤立关键词，无法理解否定/转折（"aren't waterproof"被匹成"Waterproof"→产出了"Keeps Water Out"；"wanted these to fit but..."被匹成"fit good"→产出了"Fits as Expected"）。此前 blocked_patterns 修补法永远追不上自然语言的多样性，每个新品类都要重新适配。session 126 线上验证发现 2/20 差评受影响（id=27171, 27152），cluster T2 评分守卫已拦截传播污染但这轮是规则引擎独立匹配，不受保护。 | 新增 Layer 0 极性对撞守卫（`_is_polarity_guard_enabled` / `_llm_polarity_by_aspect_key` / `_polarity_guard_rejects_candidate`），在 `_append_waders_content_rule_occurrences()` 中（规则引擎唯一收口点，读写两条路径同时生效）对撞 LLM 已判定的 aspect polarity：highlight candidate 撞 negative → 否决；issue candidate 撞 positive → 否决；neutral/LLM 未提该 aspect → 放行。cluster 传播来的极性不作为对撞依据（不是本条评论自己的判断）。通过 `CONTENT_RULE_POLARITY_GUARD_ENABLED` env（默认 true）可紧急关闭。新增 `test_content_rule_polarity_guard.py` 23 测试全绿（含 session 126 两条差评回归 + 读路径不复活动 + guard 关闭降级）。零 token 增量成本、覆盖所有品类、历史 session 打开时自动纠正。已随 commit `cc76e42` 落地。 |
...
| 2026-08-07 | fix | **模型链路海外合规改造：DeepSeek/Qwen → Gemini 2.0 Flash**。`llm_router.py` 新增 `_GEMINI`（`gemini-2.0-flash`，OpenAI 兼容端点 `generativelanguage.googleapis.com/v1beta/openai/`，threshold=3/cooldown=60s），删除 `_QWEN`，`MODELS_EN`/`MODELS_ZH` 统一为 `[_OPENAI, _GEMINI]` 双模型链。费率表（`deep_analyzer._estimate_cost` + `database.MODEL_COST_PER_MILLION`）追加 `gemini-2.0-flash`=(0.7, 2.9)，保留 deepseek/qwen 条目供历史数据用量查询。`_provider_from_model_name` 新增 gemini 识别。`insight_engine` 默认 `RESULTS_AI_DISABLED_PROVIDERS` 从 `deepseek` 改 `gemini`。`analyzer.get_api_key` 兜底 env 从 `DEEPSEEK_API_KEY` 改 `OPENAI_API_KEY`。 | ruff PASS；23 focused tests PASS |
| 2026-08-07 | fix | **L1 缓存 model 门修复（真 bug，非本次改造引入）**。`CACHE_MODEL_NAME` 原值 `"deepseek"` 与写入侧不同构：写入侧 `jobs.py:995` 存的是 `router_completion()` 返回的 **model_id**（`llm_router.py:390` `return resp, model.model_id` → `"gpt-4o-mini"`），读取侧 `database.py:1571` 用 `aspects_json->>'model_name' = %s` 等值比较 → `'gpt-4o-mini' = 'deepseek'` **永不命中**，该门自 5.9.6-B.1 上线起一直把 L1 缓存判死（另 3 个版本门正常工作）。改为 `CACHE_MODEL_NAME = "gpt-4o-mini"` 与 model_id 同构，L1 缓存真正打开。Gemini fallback 产出结果按语义判 miss（不跨模型复用），符合预期。 | ruff PASS；test_cache_semantic_versioning 5/5 PASS |
| 2026-08-07 | verify | Gemini API key 连通性验证：OpenAI 兼容端点认证通过，返回 429 `limit: 0`（免费层输入 token 配额为 0，非"用完等重置"）→ **key 有效但未开付费**，Gemini 兜底实际不可用，需在 Google AI Studio 绑卡 | ⚠️ 待 Erika 开通付费 |
| 2026-08-10 | fix | **5.9.6-C1: `_apply_enforce_filter` 修复 — 增加 `customer_label_occurrences` 过滤**。原先只过滤 `aspects_json.aspects`（V3 旧路径，数组为空），导致 enforce 模式下 registry 拒绝的 label 仍出现在客户前台 TOP10 展示中。修复后同时过滤 `aspects_json.customer_label_occurrences`（V2 实际数据路径），通过 `canonical_label_key` 匹配 `rejected_keys`，设置 `display_allowed=False` + `registry_rejected=True`。`_project_customer_label_occurrence`（line 2419）已有 `display_allowed is False` 跳过逻辑。 | 生产验证：session 126 waders 53 条 occurrence 全部 registry 通过（13 unique keys 均无拒绝），enforce filter 逻辑正确但当前无拒绝案例；修复已部署待 resolver 实际拒绝时确认生效。 |
| 2026-08-10 | fix | **Gemini 模型 ID 更新：`gemini-2.0-flash` → `gemini-flash-latest`**。Google 于 2026-08 下线了 OpenAI 兼容端点上的 `gemini-2.0-flash`（返回 404 "no longer available"），改为 `gemini-flash-latest`（始终解析为最新稳定 Flash，避免未来版本过期）。同时 Erika 重新提供 Gemini API key（原 key 配额已开通），连通性验证通过（JSON 模式 / 中文 / 结构化输出均正常）。改动范围：`llm_router.py` model_id、`deep_analyzer._estimate_cost` + `database.MODEL_COST_PER_MILLION` key 同步。 | ruff PASS；23 focused tests PASS；3 场景 live API 验证 PASS |
| 2026-08-07 | verify | 回归：`backend_api/tests/` 487 passed / 11 failed；11 项失败经 HEAD 干净 worktree 基线比对确认为**改动前既有失败**（`test_outdoor_waders_taxonomy` aspect_count 22≠21、`test_review_fragment_candidate_multimodule` 10 项），与本次改动无关 | ✅ 0 新增失败 |
| 2026-08-11 | fix | **5.9 阶段 A：Prompt 版本错配修复**。验收脚本 `acceptance_six_metrics.py` 硬编码 `PROMPT_VERSION = "v2.1"`，生产 `workers/jobs.py` 使用 `DEFAULT_ANNOTATE_VERSION = "v2.4"`。v2.1 无 `{{ASPECTS_BLOCK}}` 占位符，导致 waders taxonomy 从未注入 prompt，LLM 看到的是家具 taxonomy（含 stability）。旧 cache（`prompt_version: "v2.1"`）为"家具 prompt + waders 校验"错配。修复：① 验收脚本改从 `prompt_registry` 导入 `DEFAULT_ANNOTATE_VERSION`；② 缓存加载增加 `prompt_version` + `model_name` 校验，不匹配则拒绝；③ 缓存写入增加 `model_name` + `taxonomy_version` 字段。 | ✅ v2.4 对齐后 waders 专属 key 全量命中（waterproof:12, boot_fit:6, grip:4, breathability:3 等） |
| 2026-08-11 | fix | **5.9 阶段 A：schema_invalid 重试修复**。`deep_analyzer.analyze_one()` retry loop 中 messages 从不更新 → temperature=0 导致 LLM 重复相同非法 key。修复：schema 校验失败后向 messages 追加通用错误反馈（含错误原因、allowed keys、合法 polarity/evidence_level 值），使重试有实际信息增量。最终仍失败时返回 `error_type=schema_invalid` + `schema_errors` 明细（包含每次失败的 attempt/error/model）。新增 3 个单元测试（test_v24_dynamic_aspects.py）：非法 key 修复后成功、两次失败返回明细、不同 taxonomy 动态 allowed_aspects。 | 26/26 tests PASS；2/50 仍失败（polarity_invalid、key_invalid_'fit'） |
| 2026-08-11 | fix | **5.9 阶段 A：overall_satisfied 不进前台**。Erika 确认 `overall_satisfied` 不进入客户前台 `product_highlight`。修复：① `_project_customer_label_occurrence()` 中 `overall_satisfied` + `highlight` → return None（v2 path）；② `_normalize_aspect_highlight()` 中 `canonical=="overall_satisfied"` → `display_allowed=False`（legacy path）。后台原始数据、audit 记录保留不变。不新增 waders 正则，不影响短但具体的标签（Keeps me dry → waterproof / Very warm → temperature_rating）。 | ✅ 新 cache 中 overall_satisfied aspect key = 0 |
| 2026-08-11 | fix | **5.9 阶段 A：Gold fixture 修正**。① `overall_satisfied` 从 21 条 review 的 `expected_highlight_keys` 中移除（gold 总数 92→71）；② `runs_too_small` 替换 #366/#386 同一证据上的 `size_fit_problem`；③ fixture 新增 `_revision_notes` 字段记录全部 26 条变更及口径说明。原始 `annotated_review_labels` 保留不变（仅标记 `_replaced_by_fine_grained=True`）。 | ✅ overall_satisfied=0；细标签替换 2 条 |
| 2026-08-11 | measure | **5.9 阶段 A：v2.4 新基线六指标**。错标率 19.2%（FP=14/73）、漏标率 16.9%（FN=12/71）首次降至 20% 以下、极性反标 3（#366/#386 粗细标签重复，#378 误标 not_breathable+runs_too_large）、串台 0、模块流向正确率 92.5%（removing overall_satisfied 从 gold 消除 10 条 audit_filter 违规）、other 占比 9.9%。48/50 成功（96%），2 条 schema_invalid（polarity_invalid、key_invalid_'fit'）。旧错配基线 16.1%/20.6%/3/0/75.0%/13.9% 已作废。 | 漏标率/模块流向/other 三项首次全部过线 |
| 2026-08-11 | finding | **缺口 1（waders 专属维度=0）已消除**。v2.4 对齐后 aspect key 分布：waterproof:12、boot_fit:6、mobility:5、grip:4、breathability:3、accessory_storage:2、seam_integrity:1、temperature_rating:1。仅 weight:0（这些评论未提及）。stability 降为 0（不再出现家具 key）。**结论：缺口 1 是 prompt 版本错配造成的伪缺陷，v2.4 对齐后无需额外 prompt 调整。** | ✅ 缺口 1 关闭 |
| 2026-08-11 | finding | **剩余问题**：① FP 率 19.2% 仍高（需逐条分析 14 个 FP 原因）；② 极性反标 #366/#386 源于系统同时输出粗标签（size_fit_problem）和细标签（runs_too_small）——粗细标签同一证据并存问题需在 prompt 或后处理中解决；③ 2 条 schema_invalid 的 retry 反馈未能修复（temperature=0 下 GPT-4o-mini 确定性重复），需后续调整 max_tokens 或打开 temperature。 | ⚠️ 不阻塞阶段 C 开进 |
