...
| 日期 | 问题描述 | 解决方案 |
|------|---------|---------|
| 2026-07-28 | **50U-T1 production write-path smoke STOP after worker `done`**。PGOPTIONS 修复后，15-row production upload smoke 重新 preflight 全通过并创建 `job_id=96` / `session_id=115`，poll 约 31s 到 `done`。结果页 sanity 基本通过：comments=15、无 embedding 泄漏、Representative Evidence 可定位、无 `cluster_propagated=true` top row、无 positive waterproof evidence under `Water Leaks Through`。DB 对账：credit delta exactly -15 `review_analyze` with `ref_id=96`，LLM non-cache rows=14，cost_yuan=0.089850，`analysis_job_complete` present，products/product_variants unchanged，`review_pool` 影响限定为 approved 15 hashes，push_snapshots/action_items 无新增。最终 STOP 原因：comments processed=15 但 analyzed=14/15；一条评论在 LLM schema_invalid retries exhausted 后被 worker 写成 legacy fallback `aspects_json=NULL` 并仍标记 job done。另发现 `llm_usage_log.model_name` 记录 provider `openai`，不符合 `gpt-4o-mini/deepseek-chat/qwen-plus` 模型 ID 口径。 | 本地修复已准备但未部署/未 push：`workers/jobs.py` 的 exhausted LLM error fallback 改写结构化非空 `aspects_json`，带 `analysis_error/analysis_fallback` 标记；degraded fallback 不回填 `review_pool`。`review_analyzer/database.py` 的 content_hash cache 读取排除 `analysis_error` fallback，避免污染 user/global cache。`backend_api/app/services/llm_router.py` 成功路径返回真实 `model_id`，使后续 usage 记录为 `gpt-4o-mini` 等允许模型名。新增 focused tests；`python3 -m py_compile ...` PASS，`python3 -m pytest backend_api/tests/test_worker_error_fallback.py backend_api/tests/test_database_read_path.py -q` PASS（14 passed）。不得基于本次 session 执行 reanalysis；新的生产写路径验证需 Erika 另行授权。 |
| 2026-07-28 | **50U-T1 production `POST /uploads` 500 / no `upload_jobs` row**。production write-path smoke 在 auth、credit、product、taxonomy、duplicate-session preflight 全通过后，上传 15-row XLSX 到 `POST /uploads?locale=en` 返回 500；DB 事后对账确认 `upload_jobs/sessions/comments/credit_ledger/llm_usage_log/analytics_events/review_pool` 全部 delta=0。API 日志 traceback 关键行：`psycopg2.errors.ReadOnlySqlTransaction: cannot execute INSERT in a read-only transaction`，发生在 `review_analyzer/database.py:create_upload_job()` 第一条 `INSERT upload_jobs`。这和 2026-07-03 的“上传 500 且无 job row”同属 API pre-job failure，但本次不是缺 migration，而是 API 容器连接默认 `transaction_read_only=on` / `default_transaction_read_only=on`。诊断确认 `PGOPTIONS` 未设置、DSN query 为空、`pg_is_in_recovery=False`，`pg_settings.source=session`；手动 `SET default_transaction_read_only=off` 可恢复。 | 生产 `deploy/.env` 增加 `PGOPTIONS=-c default_transaction_read_only=off`，执行 `docker compose up -d --force-recreate api worker scheduler` 重建读取 env 的容器；复查 API 容器内 `PGOPTIONS=-c default_transaction_read_only=off`，`SHOW default_transaction_read_only=off`，`SHOW transaction_read_only=off`。未重试上传，需 Erika 重新 GO 才能继续 50U-T1。后续改进：只读 preflight 脚本不得通过共享生产 pooler 污染 session GUC；遇到 `POST /uploads` 500 且无 `upload_jobs` 时，第一时间查 API traceback 和 `SHOW transaction_read_only/default_transaction_read_only`。 |
...
| 2026-07-29 | 生产上传 preflight | `python3 tmp/production_tidewe_waders_step3_runner.py`（生产 API/DB preflight；不带 `--execute`） | STOP；`/health` 200，100 行上传样本已生成，生产登录 `[REDACTED_PROD_EMAIL]` 与 `[REDACTED_PROD_USERNAME]` 均 401 | 未执行上传；未创建 `upload_jobs` / `sessions`；未触发 `review_analyze` credit、LLM、export credit。当前阻塞：需要有效生产凭证或已认证 session/cookie。 |
| 2026-07-29 | session 118 结果页验收 | `python3 tmp/production_tidewe_waders_step3_runner.py --validate-session 118`（生产 DB 只读 + 本地等价导出；不调用上传/重分析/API export） | `REVIEW_NEEDED`；100 comments，29 focus rows all found；单条明细 no evidence count=0，cluster propagated count=0，Top violations=[]，four_pack_bad_rows=[]；产出 `docs/5.9.8-step3-tidewe-waders-session118-acceptance-report.md` 与 `tmp/5.9.8-step3-tidewe-waders-prod-20260729/*` 导出文件。 | Step 2 展示 gating 硬门禁 PASS，但语义验收未通过：row 46 phone case、row 52 旧产品、row 62 no leaks 正向语境仍污染 `water_leaks_through`；`pocket_not_waterproof` 单标签导出 rows=0；生产缺 `customer_label_catalog` 表，resolver 使用 fallback。 |
| 2026-07-29 | session 118 缓存风险复核 | 同上结果 + DB grouped usage | 证据确认存在复用：`llm_usage_log` 51 rows `provider=cache/cache_hit=true`，comments 49 rows `cache_hit_level=L1`；非缓存 LLM rows=30，cost_yuan≈0.261506；`review_analyze` credit `-100` ref_id=99。 | 因 content hash = `content + rating + category` 且不含 ASIN，同批评论换 ASIN 仍可能命中旧分析；session 118 只能验当前线上展示/导出口径，不能作为干净新分析验收。 |
| 2026-07-29 | 删除后只读清理审计 | Erika 已手动删除 `session_id=118` 产品及评论；Codex 运行 `python3 tmp/production_tidewe_waders_step3_runner.py --audit-cleanup`（生产 DB 只读） | `status=CACHE_RISK_REMAINS`；`session_118.sessions=0`、`comments_118.comments=0`、`product_sessions=[]`、`product_comments.comments=0`、`matching_hash_comments.comments=0`、`products=[]`；但 `review_pool.rows=49`、`review_pool.analyzed_rows=49`、`analyzer_version=v4_deep`。 | 测试账号用户侧 L1 已清干净；全局 `review_pool` 仍会导致下一次同批评论可能命中旧分析。干净复测前需要 Erika 明确授权是否清理这 100 条 content_hash 对应的 `review_pool` 已分析缓存。 |
| 2026-07-29 | review_pool 清缓存前只读预检 | `python3 tmp/production_tidewe_waders_step3_runner.py --audit-review-pool-cache`（生产 DB 只读；本地写审计 JSON） | `CACHE_RISK_REMAINS`；只读 flags=`transaction_read_only=on/default_transaction_read_only=on`；`review_pool_overview={rows:49, distinct_hashes:49, analyzed_rows:49, aspects_rows:49, target_rows:49}`；版本分布仅 `v4_deep`，`analyzed_at=2026-07-28 08:59:12.401966+00:00`；目标 hash 范围 `053129c2...2872e` → `fbc75dc5...1aafd`；完整明细输出到 `tmp/5.9.8-step3-tidewe-waders-prod-20260729/review-pool-cache-preclear-audit.json`。 | 预检完成但未清理。清理方案已在脚本中限定为同批 49 行 `review_pool`，只置空 `sentiment/aspects_json/analyzed_at/analyzer_version`，不删除评论源；必须等待 Erika 明确授权 `GO_DELETE_TIDEWE_WD001_CLEAN_RETEST` 后才可执行。 |
| 2026-07-29 | review_pool 授权清缓存 | `python3 tmp/production_tidewe_waders_step3_runner.py --clear-review-pool-cache --authorization GO_DELETE_TIDEWE_WD001_CLEAN_RETEST` | `CLEARED`；`updated_rows=49`，影响 id 范围 `1958..2006` 中的 49 条同批缓存；仅置空 `sentiment/aspects_json/analyzed_at/analyzer_version`，原始 `review_pool` 行保留；清理结果写入 `tmp/5.9.8-step3-tidewe-waders-prod-20260729/review-pool-cache-clear-result.json`。 | 未上传、未调用生产分析 API、未触发 credit/LLM；清理范围限定在 TIDEWE WD001 人工样本对应 content_hash。 |
| 2026-07-29 | 清缓存后只读复核 | `python3 tmp/production_tidewe_waders_step3_runner.py --audit-review-pool-cache` + `python3 tmp/production_tidewe_waders_step3_runner.py --audit-cleanup`（均为生产 DB 只读；`--audit-cleanup` 已加固为显式 `BEGIN READ ONLY`） | `review-pool-cache-postclear-audit.json` 显示 `status=NO_TARGET_CACHE`、`target_rows=0`、`review_pool_overview={rows:49, distinct_hashes:49, analyzed_rows:0, aspects_rows:0}`；`cleanup-audit-after-session118-delete.json` 显示 `status=CLEAN_FOR_RETEST`、`clean_for_user_l1=true`、`clean_for_global_l1=true`、`session_118=0`、`comments_118=0`、`matching_hash_comments.analyzed_cacheable=0`。 | 当前已满足干净重传前置条件。下一步必须由 Erika 手动在前端重传同一人工样本，并提供新 `session_id`；该上传会真实消耗 `review_analyze` credit 与 LLM 成本，Codex 不执行生产上传。 |
