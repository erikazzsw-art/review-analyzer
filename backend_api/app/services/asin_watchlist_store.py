...
def add_watchlist_items(
    ...
    try:
        with conn.cursor() as cur:
            for asin in asins:
                cur.execute(
                    """INSERT INTO asin_watchlist (user_id, asin, marketplace, fetch_frequency)
                       VALUES (%s, %s, %s, %s)
                       ON CONFLICT (user_id, asin, marketplace) DO UPDATE
                         SET status = 'active', fetch_frequency = EXCLUDED.fetch_frequency,
                             updated_at = NOW()
                       RETURNING id, asin, marketplace, product_name, product_id,
                                 fetch_frequency, last_fetched_at, last_review_count,
                                 new_review_count, status, error_message, created_at""",
                    (user_id, asin, marketplace, fetch_frequency),
                )
                row = cur.fetchone()
...
def get_watchlist(user_id: int) -> list[dict[str, Any]]:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT id, asin, marketplace, product_name, product_id,
                          fetch_frequency, last_fetched_at, last_review_count,
                          new_review_count, status, error_message, created_at
                   FROM asin_watchlist
                   WHERE user_id = %s
...
def get_watchlist_item_by_id(item_id: int) -> dict[str, Any] | None:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT id, user_id, asin, marketplace, product_name, product_id,
                          fetch_frequency, last_fetched_at, last_review_count,
                          new_review_count, status, error_message, consecutive_empty,
                          created_at
                   FROM asin_watchlist
                   WHERE id = %s""",
                (item_id,),
...
def get_active_items_for_schedule(frequency: str) -> list[dict[str, Any]]:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """SELECT w.id, w.user_id, w.asin, w.marketplace, w.product_name,
                          w.product_id, w.last_fetched_at, w.last_review_count,
                          w.consecutive_empty
...
def mark_fetch_error(item_id: int, error_msg: str) -> None:
    conn = get_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
