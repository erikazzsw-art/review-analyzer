...
- [ ] **Step 9.1: Review Signal Layer for Customer Label Routing Fix（Step 9 人工验收失败后的上线前修复项）**
  ...
  - **Phase 状态（2026-07-31）：**
    ...
    - [ ] **Phase 4 / frontstage guarded read path**
      - [x] `Phase 4.0 design/readiness`：方案 artifact `review-signal-phase4-guarded-read-path-plan.json` 为 `READY_FOR_ERIKA_REVIEW_DESIGN_ONLY`；readiness checklist artifact `review-signal-phase4-readiness-checklist.json` 为 `READY_FOR_IMPLEMENTATION_AUTHORIZATION`。本阶段只完成设计/准备，不等于 runtime 已接入。
      - [x] `Phase 4.1 mapping safety gate`：Erika 已完成 14 条 unresolved mapping review；`map_to_existing_canonical=4`、`propose_new_candidate_label=9`、`keep_audit_only=1`、`needs_erika_review=0`。`candidate:*` 仍不是正式 canonical、不计 PASS、不进 frontstage。
      - [ ] `Phase 4.2 implementation authorization gate`：真正改 frontstage / read path / build_results_insights 前，必须由 Erika 单独授权；未授权前只能更新 docs/artifact。
      - [ ] `Phase 4.3 working tree isolation`：开工前必须隔离 unrelated dirty files，特别确认不混入既有 `backend_api/app/services/customer_label_v2_frontstage.py` 未提交改动；不得顺手改 `specific_issue.py` 普通规则。
      - [ ] `Phase 4.4 stored shadow input contract`：前台 guarded read 只能消费已存 shadow/read model；不得在前台 read path 触发 runtime shadow、真实 LLM、upload、reanalysis、credit 或 production DB write。
      - [ ] `Phase 4.5 product-derived display selector`：Customer Issue 只消费 `product_negative`；Customer Label 只消费 `product_positive`；`comparison_or_other_product` / `accessory_only` / `shipping_service` / `generic_or_vague` / `audit_only` 只能 audit/filter。
      - [ ] `Phase 4.6 non-product insight routing`：`audience` / `usage_location` / `usage_time` / `behavior` -> `consumer_profile`；`purchase_motivation` -> `purchase_motives`；`expectation` -> `unmet_needs`；这些信号不得进入 Customer Issue / Customer Label Top10。
      - [ ] `Phase 4.7 four frontstage consumers`：Results Top10、single review detail、raw review export、single-tag download 必须共享同一个 selected product-derived display occurrence set；candidate/audit/non-product signal 不出现在主 issue/label 字段。
      - [ ] `Phase 4.8 feature flag / scope / defaults`：feature flag 默认 off；仅支持 Erika 授权的明确 session/category/sub_category scope；invalid config、missing stored shadow、unresolved mapping、candidate key、evidence gate failure 一律 fail closed 到 `v1_current` 或 exclude occurrence。
      - [ ] `Phase 4.9 rollback / kill switch`：global rollback、scoped rollback、global kill switch、scoped kill switch 都必须强制回 `v1_current`，且优先级高于 feature flag。
      - [ ] `Phase 4.10 observability`：必须输出 read-path selected、blocked reasons、routing leakage、candidate-key frontstage count、evidence-not-found count、stored shadow availability、rollback/kill switch counters；硬阈值为 leakage `0`、candidate frontstage `0`、evidence-not-found frontstage `0`。
      - [ ] `Phase 4.11 local acceptance`：focused unit tests、四条 frontstage consumer tests、shadow replay、ruff、compileall、`git diff --check` 全部通过后，Phase 4 才能从 implementation 进入 local acceptance done。
      - [ ] `Phase 4.12 post-implementation safety check`：grep runtime imports / upload / reanalysis / credit / LLM / feature flag / deploy scope，确认没有超出授权范围；输出 Phase 4 implementation artifact 与 safety artifact。
    - [ ] **Phase 5 / production truth check replay**
      - [ ] `Phase 5.0 authorization gate`：只有 Phase 4 implementation + local acceptance + safety check 全部通过后，且 Erika 明确授权，才可进入 Phase 5；当前不得上传生产样本、不得打开或扩大 feature flag。
      - [ ] `Phase 5.1 preflight`：确认 HEAD/CI/deploy 状态、feature flag 默认 off、rollback/kill switch 可用、stored shadow/read model 可用、观测 scope 明确；输出 preflight artifact。
      - [ ] `Phase 5.2 production baseline / stored-shadow observation`：在 Erika 授权范围内只读观察 production session/job/comment/read model/observability；不写库、不重分析、不扣 credit、不调用真实 LLM。
      - [ ] `Phase 5.3 scoped replay / comparison`：只在授权 session 或 scope 内对比 guarded read path 与当前前台结果，检查 Customer Issue FP/FN、Customer Label FP/FN、routing leakage、candidate/audit leakage、evidence 是否存在。
      - [ ] `Phase 5.4 four-path truth check`：Erika 或授权只读流程检查 Results Top10、single review detail、raw review export、single-tag download；四条路径必须与 selected product-derived display occurrences 一致。
      - [ ] `Phase 5.5 non-product routing verification`：确认 audience/context/behavior/motivation/expectation 没有误进 issue/label；确认 shipping/accessory/other-product/generic/audit_only 没有用户可见 issue/label 展示。
      - [ ] `Phase 5.6 safety thresholds`：若 leakage > `0`、candidate frontstage > `0`、evidence-not-found frontstage > `0`、关键 FP/FN 回归、scope 异常或 observability 缺失，则结论必须是 `FIX_REQUIRED_BEFORE_GRAY_EXPAND` 或 `ROLLBACK_REQUIRED`。
      - [ ] `Phase 5.7 decision artifact`：输出 production truth check replay artifact；结论只能是 `PASS`、`FIX_REQUIRED_BEFORE_GRAY_EXPAND`、`ROLLBACK_REQUIRED` 或 `BLOCKED_NEEDS_AUTH_OR_STORED_SHADOW`。
      - [ ] `Phase 5.8 post-run boundary`：Phase 5 PASS 也不自动等于 gray expand；任何生产开关、scope 扩大、真实上传/重分析或部署仍需 Erika 再次单独授权。
  - 当前结果页模块核对：
...
- [x] 完成 `workers/alert_checker.py`：每 10 分钟扫描错误率、P95、成本、缓存、熔断、卡死任务
- [x] 新增/完善 `GET/PUT /analytics/alert-config`：支持阈值配置、开关、Webhook 目标、Redis TTL 去重
- [x] Alerts Tab 从占位改为可用：展示当前触发中的告警、历史告警、阈值配置、最近发送时间
- [x] 任务列表增加失败阶段、错误类型、可复制 Job ID、关联 session_id/product_id、是否扣费/是否部分完成
- [x] 验收：本地模拟发送/未配置 Webhook/去重路径通过；真实飞书群未发送测试消息（未配置或请求真实 Webhook）

**2026-07-29 P1 完成记录：**
- Worker：新增 `workers/alert_checker.py`，按用户配置扫描 `llm_call` 错误率/P95、用户与系统日成本、24h 缓存节省率、processing 卡死任务、模型熔断；scheduler 每 10 分钟入队；Redis `SET NX EX` 做 TTL 去重，Redis 不可用时回退 `analytics_events` 近期历史去重。
- 后端：新增 `observability_alert_config` settings 配置，`GET/PUT /analytics/alert-config` 返回配置、当前触发、历史告警、最近发送时间；告警历史复用 `analytics_events(event_name='observability_alert')`。
- 前端：Alerts Tab 改为可用配置/当前告警/历史告警视图；Jobs Tab 增加后端状态筛选、失败阶段、错误类型、复制 Job ID、session_id/product_id、扣费与部分完成标记。
