...
- [x] **A-3.1** 实现同证据去重后处理：同一 `evidence_span` 命中父子/粗细关系的两个标签时，只保留细粒度标签（走"只否决不创造"的规则口径，不改极性判断） — **✅ 2026-08-12 完成**：`label_deduplication.py` + `specific_issue.py` 10 个 return 接入点 + 36 单元测试
