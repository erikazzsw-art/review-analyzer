...
| 2026-07-31 | v3-lite-develop-cleanup | 按 Erika 授权执行非破坏性迁出：已创建并推送远端隔离分支 `feature/5.9.9-v3-lite-shadow-only` 保存 v3-lite 现状；develop cleanup 移除生产包内 tracked v3-lite 文件：`backend_api/app/services/customer_insights_v3_lite.py`、`backend_api/tests/test_customer_insights_v3_lite.py`、`scripts/customer_insights_v3_lite_shadow_replay.py`、`docs/5.9.9-v3-lite-customer-insights-shadow.md`。 | cleanup commit / deploy clean develop 待执行；完成前 Step 9 仍 `FIX_REQUIRED_BEFORE_GRAY_EXPAND`，不得扩大 v2 gray exposure。 |
