...
| Step 4 waders 盲测与真实上传验收 | session121 本地规则 PASS；session121 生产下载结论作废（CI/CD 未成功部署）；session122 生产下载 PASS（raw/audit 分列已落地） | 验证不是“背答案”，用 50 条从未上传过的 waders 样本和真实上传判断标签是否达到用户可用；session121 本地最新规则已无 Top/单条/evidence/cluster P0，session122 下载审计 `status=PASS`、`frontstage_verified_violations=[]`，Raw Reviews 为 15 列、Label Audit 为 30 列 | `docs/5.9.8-step4-tidewe-waders-label-correctness-fix.md`、`docs/5.9.8-step4.1-tidewe-waders-session120-recall-fix.md`、`docs/5.9.8-step4-tidewe-waders-session121-blind-acceptance-report.md`、`docs/5.9.8-step4-tidewe-waders-session122-blind-acceptance-report.md`、focused gold fixture、AI vs 人工对比、真实上传抽查表、验收结论 | 以 session122 的已部署版本为准继续核验 Top10、单条明细、raw review 下载、single-tag download 是否全部只用 verified occurrence；未完成项仍保持 REVIEW_NEEDED，避免把 session121 的失效结果误当成部署后结论 |
