...
- [x] `llm_router.py` — 新增 `_GEMINI`（`gemini-flash-latest` / `generativelanguage.googleapis.com/v1beta/openai/` / `Gemini_API_KEY` / threshold=3 / cooldown=60s）；删除 `_QWEN` 与 `_DEEPSEEK`；`MODELS_EN` = `MODELS_ZH` = `[_OPENAI, _GEMINI]`；`MODELS` 默认指向 `MODELS_EN`
- [x] `deep_analyzer.py` `_estimate_cost()` + `database.py` `MODEL_COST_PER_MILLION` — 追加 `gemini-flash-latest`=(0.7, 2.9)（$0.10/$0.40 per M）；保留 deepseek/qwen 条目供历史 llm_usage 行查询
...
- [x] 2026-08-10：Gemini 模型 ID 更新 `gemini-2.0-flash` → `gemini-flash-latest`（Google 2026-08 下线旧模型返回 404），API key 连通性重新验证通过（JSON/中文/结构化输出均正常）
- [x] **Gemini 付费已开通**：Erika 重新提供 key，配额可用，fallback 链 OpenAPI + Gemini 双模型真正就绪
