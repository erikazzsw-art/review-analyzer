...
| 5.9.6-C1 客户前台证据展示 | 部分完成（内部告警已下架✅，证据展示未开始） | 客户要的是"凭什么这么判"，不是我们的模型内部状态 | 只做原文证据：TOP10 每行可展开代表性评论原文，只用 scope gate 通过的正式标签。**置信度与 `other` 占比不出现在客户 UI**；waders 旧规则继续作为冻结安全补丁，不新增规则、不删除逻辑；不得在 Registrar scope gate 完成前新增正式标签展示入口 | 0.5 天 | 用户可见原文证据片段；客户 UI 无置信度、无 `other` 占比；waders 旧补丁未扩大；前台展示不能绕过 scope gate |
...
| 5.9.7-T3 label registry 前台灰度 + enforce 交付 | 未开始（依赖 T2 完成） | 让 registry scope gate 从 off 走到单类目 enforce，用户真正受益 | 分三块：A. 给 `label_registry_frontstage.py` 的 flag 加 scope 支持（照抄 `customer_label_v2_frontstage.py` 的 session/category/sub_category scope + kill switch + rollback + fail-closed 范式，默认全部 off）；B. 真正交付 enforce（删掉 118-124 行 fallback-to-shadow，resolver 决定展示、旧路径只作 fallback 日志，resolver 异常时 fail-closed 回旧路径，kill switch 一条环境变量回 off）；C. 5.9.6-C1 客户前台证据展示（只做原文证据，置信度与 other 占比不进客户 UI，waders 旧规则冻结不扩大）。**5.9.6-C2 内部校准信号接入不并入 T3，单独排**（2026-08-07 决策：C2 需新增 `new_aspect` 提案动作，是比 0.5 天大的活，塞进 T3 会让 enforce 灰度被校准页开发阻塞）。单类目灰度 waders：off→shadow 观察 diff→确认无异常后 enforce | 2-3 天 | registry 治理在 waders 类目对用户可见；shadow diff 无异常；enforce 后 TOP10 只展示 scope gate 通过的标签；kill switch 可一键回退；waders 旧补丁未扩大 |
