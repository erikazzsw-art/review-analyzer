...
- [ ] **Step 9.1: Review Signal Layer for Customer Label Routing Fix（Step 9 人工验收失败后的上线前修复项）**
  ...
  - **Phase 状态（2026-07-31）：**
    ...
    - [ ] **Phase 4 / frontstage guarded read path**：方案 artifact 已准备为 `READY_FOR_ERIKA_REVIEW_DESIGN_ONLY`，设计为 Customer Issue 只消费 `product_negative`、Customer Label 只消费 `product_positive`，非产品信号流向 consumer_profile / purchase_motives / unmet_needs / audit/filter；当前仍未接前台、未开 feature flag，真正实现需 Erika 单独授权。
    - [ ] **Phase 5 / production truth check replay**：Phase 4 通过且 Erika 单独授权后再执行；当前不得上传生产样本。
  - 当前结果页模块核对：
